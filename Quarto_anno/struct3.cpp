...
void eventopartecipanti(Evento eventi[], int n){
    int maxpartecipanti = eventi[0].partecipanti;
    int k = 0;
    for (int i = 0; i < n; i++){
        if (eventi[i].partecipanti>maxpartecipanti){
        maxpartecipanti=eventi[i].partecipanti;
        k = i;
        }
    }
    cout << "l' evento con piu' partcipanti ha " << maxpartecipanti << " ed ha il titolo " << eventi[k].titolo << endl;
}
...
void eventoprimadopo(Evento eventi[], int n){
    ...
    if (eventi[k1].data.anno < eventi[k2].data.anno){
        cout << eventi[k1].titolo << " si svolge prima di " << eventi[k2].titolo << endl;
    }
    else if (eventi[k1].data.anno > eventi[k2].data.anno){
        cout << eventi[k1].titolo << " si svolge dopo " << eventi[k2].titolo << endl;
    }
    else if (eventi[k1].data.mese < eventi[k2].data.mese){
        cout << eventi[k1].titolo << " si svolge prima di " << eventi[k2].titolo << endl;
    }
    else if (eventi[k1].data.mese > eventi[k2].data.mese){
        cout << eventi[k1].titolo << " si svolge dopo " << eventi[k2].titolo << endl;
    }
    else if (eventi[k1].data.giorno < eventi[k2].data.giorno){
        cout << eventi[k1].titolo << " si svolge prima di " << eventi[k2].titolo << endl;
    }
    else if (eventi[k1].data.giorno > eventi[k2].data.giorno){
        cout << eventi[k1].titolo << " si svolge dopo " << eventi[k2].titolo << endl;
